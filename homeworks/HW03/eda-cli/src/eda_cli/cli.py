...
def report(
    ...
    title: str = typer.Option("EDA-отчёт", help="Заголовок отчёта (для report.md)."),
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags.get('has_constant_columns'):
             f.write(f"- Константные колонки: Да. Список: {quality_flags['constant_columns_list']}\n")
        if quality_flags.get('has_high_cardinality_categoricals'):
             f.write(f"- Высокая кардинальность: Да. Список: {quality_flags['high_cardinality_list']}\n")
        f.write("\n")
