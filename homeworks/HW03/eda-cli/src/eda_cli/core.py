...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    constant_cols_list = [c.name for c in summary.columns if c.unique <= 1]
    flags['has_constant_columns'] = len(constant_cols_list) > 0
    flags['constant_columns_list'] = constant_cols_list
    ...
    flags['has_high_cardinality_categoricals'] = len(high_card_cols_list) > 0
    flags['high_cardinality_list'] = high_card_cols_list
    ...
    # Штраф за одинаковые значения
    if flags['has_constant_columns']:
        score -= 0.15
        
    # Штраф за большое число уникальных значений
