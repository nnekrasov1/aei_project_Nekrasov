...
@app.post(
    ...
    response_model=dict,
    tags=["quality"],
    summary="Возвращает только флаги качества по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    """
    Эндпоинт для получения полного набора флагов качества данных из CSV,
    ...
    flags_output = {
        ...
        "constant_columns_list": flags_all.get("constant_columns_list", []),
        "has_high_cardinality_categoricals": flags_all.get("has_high_cardinality_categoricals", False),
        "high_cardinality_list": flags_all.get("high_cardinality_list", []),
    }

    print(f"[quality-flags-from-csv] filename={file.filename!r} flags={list(flags_output.keys())}")

    return {"flags": flags_output}